  ...
  return (
    ...
    >
      ...
      <div className={`sticky top-[calc(var(--header-offset)/2)] flex h-[calc(100vh-6rem)] items-center`}>
        <div className={`${shouldReduceMotion ? "overflow-x-auto snap-x snap-mandatory" : "overflow-hidden"} relative w-full`}>
          <motion.div style={shouldReduceMotion ? undefined : { x }} className="flex gap-8 pr-8 will-change-transform">
            {t.scenes.map((scene) => (
              <article
                key={scene.title}
