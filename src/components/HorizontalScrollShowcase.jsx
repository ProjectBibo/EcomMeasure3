...
export default function HorizontalScrollShowcase() {
  ...
  const renderScene = (scene) => (
    <article
      key={scene.title}
      className="group relative flex w-full flex-shrink-0 flex-col justify-between rounded-[2.5rem] border border-white/60 bg-white/55 p-8 shadow-[12px_24px_60px_rgba(15,23,42,0.12)] backdrop-blur-xl transition-transform duration-500   "
    >
      <div className="absolute inset-0 rounded-[2.5rem] bg-gradient-to-br from-white/50 via-white/10 to-white/5 opacity-0 transition-opacity duration-500 group-hover:opacity-100   " aria-hidden />
      <div className="relative flex flex-col gap-4">
        <span className="inline-flex w-max items-center gap-2 rounded-full bg-white/70 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.28em] text-neutral-600 shadow-[inset_0_1px_0_rgba(255,255,255,0.9)] backdrop-blur-sm  ">
          {scene.eyebrow}
        </span>
        <h3 className="text-2xl font-semibold text-neutral-900 ">{scene.title}</h3>
        <p className="text-sm leading-relaxed text-neutral-600 ">{scene.description}</p>
      </div>
      <div className="relative mt-10">
        <div className="inline-flex flex-col items-start gap-1 rounded-2xl bg-gradient-to-br from-brand-blue to-brand-teal px-4 py-3 text-white shadow-[0_16px_35px_rgba(15,23,42,0.18)] ">
      ...
      <div className="pointer-events-none absolute -inset-[1px] rounded-[2.6rem] shadow-[inset_2px_2px_6px_rgba(255,255,255,0.65),inset_-4px_-12px_24px_rgba(148,163,184,0.25)] transition-opacity duration-500 group-hover:opacity-100 " aria-hidden />
    </article>
  );
  ...
  return (
    <section
      ...
      className="relative bg-gradient-to-b from-white/40 via-white/30 to-white/40 py-20    sm:py-24"
    >
