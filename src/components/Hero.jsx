...
const gradientHeadlineClass = "text-brand-blue dark:text-brand-blue";
...
export default function Hero() {
  ...
  return (
    <section id="hero" data-snap-section className="relative isolate">
      <div className="relative page-container section-padding flex flex-col items-center text-center gap-10 md:gap-14 bg-transparent vt-hero-visual">
        <motion.span
          initial={shouldReduceMotion ? false : { opacity: 0, y: -12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={shouldReduceMotion ? undefined : { duration: 0.7, ease: "easeOut" }}
          className="inline-flex items-center gap-2 rounded-full border border-neutral-200 bg-white px-4 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-neutral-700 shadow-sm"
        >
        ...
        <motion.h1
          ...
          className="text-balance text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-semibold tracking-tight leading-tight text-neutral-900 dark:text-white vt-hero-title focus:outline-none"
          data-focus-target
          tabIndex={-1}
        >
          {shouldReduceMotion ? (
        ...
        >
          {t.stats.map((item) => (
            <div
              key={item.label}
              data-tilt-card
              className="surface-card group relative overflow-hidden px-6 py-6 text-left"
            >
              <div className="text-xs font-medium uppercase tracking-[0.24em] text-neutral-500 dark:text-gray-400">{item.label}</div>
              <div className="mt-3 text-3xl font-semibold text-brand-blue dark:text-brand-blue">{item.value}</div>
        ...
        <motion.div
          ...
          data-tilt-card
          className="vt-hero-media relative w-full rounded-3xl border border-neutral-200 bg-white px-6 py-8 text-left shadow-sm transition-[box-shadow,transform] duration-300 focus-visible:shadow-lg dark:border-white/10 dark:bg-surface-muted"
        >
          <div className="grid gap-6 md:grid-cols-2 md:gap-12">
            {t.storyline.map((story) => (
              <div key={story.title} className="relative pl-5">
                <span className="absolute left-0 top-1 h-8 w-0.5 rounded-full bg-brand-blue" aria-hidden />
