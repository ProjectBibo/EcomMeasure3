...
/**
 * Scrollytelling “Portal Edition” — DARK-AWARE + NO OVERLAP + DARK TEXT IN LIGHT MODE
 * - Portal-layers kleuren mee met theme (light/dark)
 * - Strikte scene-sequentie (geen overlap)
 * - Tekst in scenes: donker in light mode, licht in dark mode
 */
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const sceneStrict = (
        ...
        const L = tl.duration();
        tl.addLabel(`S${L}`);
        tl.set(el, { autoAlpha: 0, display: "block" }, `S${L}`);
  ...
  return (
    <section ref={root} className="relative">
      ...
      <div className="sticky top-0 h-screen overflow-hidden pt-24">
        {/* 1) Claim — donker in light, licht in dark */}
        <div ref={sClaim} className="scene absolute inset-0 grid place-items-center">
          <div className="text-center px-6 leading-[1.07] select-none">
            <h1 className="text-5xl sm:text-6xl md:text-7xl font-extrabold tracking-tight text-neutral-900 dark:text-gray-100">
        ...
        {/* 3) KPI XXL — geen blend, hoge leesbaarheid */}
