...
export default function Hero() {
  ...
  return (
    <section id="hero" data-snap-section className="relative isolate overflow-hidden section-block">
      ...
      <div className="relative content-shell flex flex-col items-center text-center gap-10 pt-10 sm:pt-14 pb-6 vt-hero-visual">
        <motion.span
          initial={shouldReduceMotion ? false : { opacity: 0, y: -12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={shouldReduceMotion ? undefined : { duration: 0.7, ease: "easeOut" }}
          className="eyebrow shadow-sm backdrop-blur bg-white/90 dark:bg-white/10 dark:border-white/15"
        >
        ...
        >
          <MotionLink
            ...
            to="/contact"
            className="btn-primary text-sm uppercase tracking-wide shadow-lg"
        ...
        >
          {t.stats.map((item) => (
            ...
            >
              <div className="text-xs font-semibold uppercase tracking-[0.2em] text-neutral-500 dark:text-gray-400">{item.label}</div>
              <div className="mt-2 text-3xl font-bold text-brand-blue dark:text-brand-blue">{item.value}</div>
        ...
        <motion.div
          ...
          data-tilt-card
          className="vt-hero-media relative w-full rounded-3xl border border-neutral-200/70 bg-white/85 px-6 py-8 text-left shadow-[18px_32px_72px_rgba(15,23,42,0.14)] backdrop-blur-xl transition-[box-shadow,transform] duration-500 focus-visible:shadow-[22px_36px_90px_rgba(15,23,42,0.2)] dark:border-white/10 dark:bg-white/5 dark:shadow-[18px_30px_80px_rgba(2,6,23,0.55)]"
        >
          <div className="absolute -left-10 top-1/2 hidden h-48 w-48 -translate-y-1/2 rounded-full border border-brand-blue/20 md:block" style={{ animation: "pulse-ring 3.5s infinite" }} aria-hidden />
