...
export default function USP() {
  ...
  return (
    <section id="diensten" data-snap-section className="relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-white/80 via-white/60 to-white/80 " aria-hidden />
      <div className="relative max-w-6xl mx-auto px-6 py-24 sm:py-28">
        <div className="grid gap-12 md:grid-cols-[minmax(0,1.1fr)_minmax(0,1.7fr)] items-start">
          <motion.div
            ...
            className="relative rounded-3xl border border-neutral-200/70 bg-white/80 p-8 shadow-lg backdrop-blur "
          >
            ...
            <AnimatedHeading
              as="h2"
              className="mt-4 text-balance text-3xl sm:text-4xl text-neutral-900 "
              delay={0.15}
            >
              {t.heading}
            </AnimatedHeading>
            ...
            {t.cards.map((item, index) => {
              ...
              return (
                <motion.article
                  ...
                  whileHover={{ y: -8 }}
                  className="group relative overflow-hidden rounded-2xl border border-white/70 bg-white/80 p-7 shadow-sm backdrop-blur transition "
                >
                  ...
                  <h3 className="typography-subheading mt-6 text-lg font-semibold text-neutral-900 ">{item.title}</h3>
                  <p className="mt-3 text-sm text-neutral-600 ">{item.text}</p>
                  <p className="mt-3 text-xs uppercase tracking-[0.18em] text-neutral-500 ">{item.detail}</p>
                </motion.article>
              );
            })}
          </div>
