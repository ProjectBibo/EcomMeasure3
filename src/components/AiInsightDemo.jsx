  ...
  return (
    <section className="section-shell">
      <div className="layout-shell">
        <div className="relative mx-auto flex w-full max-w-5xl flex-col overflow-hidden card-surface bg-white/95 p-8 md:p-10 dark:bg-white/5">
          ...
          <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <span className="eyebrow shadow-sm ring-1 ring-white/70 backdrop-blur dark:ring-white/10">
                <Sparkles className="h-3.5 w-3.5" aria-hidden="true" />
                {copy.badge}
              </span>
              <h2 className="mt-4 text-2xl font-semibold text-slate-900 dark:text-slate-100 sm:text-3xl">{copy.title}</h2>
              <p className="mt-2 max-w-2xl text-base text-slate-600 dark:text-slate-300 prose-limiter">{copy.description}</p>
            </div>
          </div>
          <form className="flex flex-col gap-4 md:flex-row" onSubmit={handleSubmit}>
            ...
            <div className="md:self-end">
              <button type="submit" className="button-primary px-5 py-3 w-full">
                {copy.button}
              </button>
            </div>
          </form>
