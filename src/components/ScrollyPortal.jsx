...
export default function ScrollyPortal() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Hoofd-timeline (korter zodat scene 2 sneller komt)
      ...
      tl.to(
        bgRadial.current,
        { background: palette.radialB, duration: 0.45 },
        "<+0.05"
      );
      tl.to(
        bgConic.current,
        { background: palette.conicB, rotate: "+=36", duration: 0.55 },
        "<"
      );
      ...
      sceneStrict(
        sSub.current,
        { autoAlpha: 0, scale: 0.94, y: 70 },
        { autoAlpha: 1, scale: 1.08, y: 0 }
      );
      ...
      tl.fromTo(
        ...
        { autoAlpha: 1, y: 0, scale: 1.06, duration: 0.55 },
        `CASE${Lcase}`
      );
      tl.fromTo(
        caseAfter.current,
        ...
        >
          <div>
            ...
            <p className="text-xl md:text-2xl text-neutral-800 dark:text-gray-200">
              Dankzij <strong>Google Analytics 4</strong>, <strong>SEO</strong>{" "}
              en <strong>trend-research</strong> maak je keuzes die winnen.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="absolute inset-0 grid place-items-center">
            <div className="px-6 py-3 rounded bg-black/50 text-white text-lg md:text-2xl">
              <strong>VIDEO PLACEHOLDER</strong> — voeg <code>/about.mp4</code>{" "}
              toe in <code>/public</code>
            </div>
          </div>
        </div>
